...
Add sample tickets for the analytics dashboard:

```bash
npm run seed:sample --prefix backend
```

Backfill activity timeline events for existing tickets:

```bash
npm run backfill:activity --prefix backend
```
...
SQLite note: Deskline stores data in `backend/data/deskline.sqlite`. If you want data to survive deploys/restarts, attach a persistent disk on Render and configure the app to use that disk path in a future production hardening step. For a short demo, the file-based database is fine.
...
After deploying Vercel, copy the Vercel URL into Render as `FRONTEND_URL`, then redeploy the Render backend.
